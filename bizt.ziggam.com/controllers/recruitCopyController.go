...
type RecruitCopyController struct {
	BaseController
}

func (c *RecruitCopyController) Get() {
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			selUpJobGrpCd = procRset.Row[17].(string)
			selJobGrpCd = procRset.Row[18].(string)
			...
			recruitModify = append(recruitModify, models.RecruitModify{
				...
				SelUpJobGrpCd:  selUpJobGrpCd,
				SelJobGrpCd:    selJobGrpCd,
	...
	fmt.Printf("jobfairMngCd:" + jobfairMngCd)
	fmt.Printf("carrGbnCd:" + carrGbnCd)
	fmt.Printf("entpAddr:" + entpAddr)
	fmt.Printf("emplTypeCd:" + emplTypeCd)
	fmt.Printf("lstEduGbnCd:" + lstEduGbnCd)
	fmt.Printf("prgsStatStep:" + prgsStatStep)
	fmt.Printf("annualSalary:" + annualSalary)
	fmt.Printf("workDays:" + workDays)
	fmt.Printf("welfare:" + welfare)
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			jobGrpList = append(jobGrpList, models.JobGrpList{
				...
				JobGrpNm:      jobGrpNm,
				ChkUpJobGrpCd: selUpJobGrpCd,
				ChkJobGrpCd:   selJobGrpCd,
			})
		}
		if err := procRset.Err(); err != nil {
			panic(err)
	...
	c.Data["SelUpJobGrpCd"] = selUpJobGrpCd
	c.Data["SelJobGrpCd"] = selJobGrpCd
	...
	c.TplName = "recruit/recruit_copy.html"
}
