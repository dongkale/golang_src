...
// LiveNvNReqAddedPopupController ...
type LiveNvNReqAddedPopupController struct {
	BaseController
}

// Get ...
func (c *LiveNvNReqAddedPopupController) Get() {
	...
	// pCheckApplyCount := c.GetString("check_apply_cnt")
	// pCheckMemCount := c.GetString("check_mem_cnt")

	var pCheckCount string

	var iCheckApplyCount int64
	var iCheckMemCount int64
	...
	if pReqType == "01" {
		...
		pCheckCount = c.GetString("check_cnt")
		fmt.Printf(pCheckCount)
		...
		if procRset.IsOpen() {
			...
			iCheckApplyCount = rslApplyRegCnt
			fmt.Printf(fmt.Sprintf("iCheckApplyCount: %v", iCheckApplyCount))
		}
		...
		if procRset.IsOpen() {
			...
			iCheckMemCount = etMemRegCnt
			fmt.Printf(fmt.Sprintf("iCheckMemCount: %v", iCheckApplyCount))
		}
	...
	c.Data["RecrutTitle"] = pRecrutTitle
	c.Data["LiveSn"] = pLiveSn
	c.Data["RecruitApplyList"] = recruitApplyList

	c.Data["ReqType"] = pReqType
	c.Data["CheckApplyCount"] = iCheckApplyCount
	c.Data["CheckMemCount"] = iCheckMemCount
	...
	c.TplName = "live/live_nvn_req_added_popup.html"
}
