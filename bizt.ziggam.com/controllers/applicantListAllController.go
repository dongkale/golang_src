...
type ApplicantListAllController struct {
	BaseController
}

func (c *ApplicantListAllController) Get() {
	...
	c.Data["TMenuId"] = "A00"
	c.Data["SMenuId"] = "A00"
	c.TplName = "applicant/applicant_list_all.html"
}

func (c *ApplicantListAllController) Post() {
	...
	pNotEntpMemNo := c.GetString("not_entp_mem_no")   // 기업회원번호
	pRecrutSn := c.GetString("recrut_sn")             // 채용일련번호
	pEvlPrgsStat := c.GetString("evl_prgs_stat")      // 평가진행상태 (00:전체, 02:대기, 03:합격, 04:불합격)
	pSex := c.GetString("sex")                        //성별 (무관:A, 남성:M, 여성:F)
	pAge := c.GetString("age")                        //연령 (전체:00, 19세이하:01, 20~29:02, 30~39:03, 40~49:04, 50~59:05, 60세이상:06)
	pVpYn := c.GetString("vp_yn")                     //영상프로필 (전체:9, 있음:1, 없음:0)
	pOneWayYn := c.GetString("oneway_yn")             //원웨이 영상 (전체:9, 있음:1, 없음:0)
	pFavrAplyPp := c.GetString("favr_aply_pp")        //관심지원자 (전체:9, 있음:1, 없음:0)
	pSortGbn := c.GetString("sort_gbn")               // 정렬구분(01:신규순, 02:마감순)
	pKeyword := c.GetString("keyword")                // 검색어
	pLiveReqStatCd := c.GetString("live_req_stat_cd") // 라이브요청상태코드(전체:A, 03:예정)
	...
	pMaxLimit := c.GetString("max_limit")
	if pMaxLimit == "" {
		pMaxLimit = "1000"
	}
	...
	// maxLimit, err := strconv.ParseInt(pMaxLimit, 10, 64)
	// if err != nil {
	// 	//
	// }

	// Start : Recruit Stat List
	fmt.Printf(fmt.Sprintf("CALL ZSP_APPLICANT_LIST_ALL('%v', %v, %v, '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', %v, :1)",
		pLang, pOffSet, pLimit, pEntpMemNo, pNotEntpMemNo, pRecrutSn, pEvlPrgsStat, pSex, pAge, pVpYn, pOneWayYn, pFavrAplyPp, pSortGbn, pKeyword, pLiveReqStatCd, pJobGrpCd, pApplySortCd, pApplySortWay, pMaxLimit))

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL ZSP_APPLICANT_LIST_ALL('%v', %v, %v, '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', %v, :1)",
		pLang, pOffSet, pLimit, pEntpMemNo, pNotEntpMemNo, pRecrutSn, pEvlPrgsStat, pSex, pAge, pVpYn, pOneWayYn, pFavrAplyPp, pSortGbn, pKeyword, pLiveReqStatCd, pJobGrpCd, pApplySortCd, pApplySortWay, pMaxLimit),
		ora.I64, /* TOT_CNT */
		ora.S,   /* ENTP_MEM_NO */
		ora.S,   /* RECRUT_SN */
	...
	rtnRecruitStatList := models.RtnRecruitStatListAll{}
	recruitStatList := make([]models.RecruitStatListAll, 0)
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			// Start : ZSP_QST_ANS_LIST_R
			fmt.Printf(fmt.Sprintf("CALL ZSP_QST_ANS_LIST_R('%v', '%v', '%v', '%v', :1)",
				pLang, rslEntpMemNo, rslRecrutSn, rslPpMemNo))

			stmtProcCallSub, errSub := ses.Prep(fmt.Sprintf("CALL ZSP_QST_ANS_LIST_R('%v', '%v', '%v', '%v', :1)",
				pLang, rslEntpMemNo, rslRecrutSn, rslPpMemNo),
				ora.S,   /* ENTP_MEM_NO */
				ora.S,   /* RECRUT_SN */
				ora.S,   /* QST_SN */
			...
			defer stmtProcCallSub.Close()
			if errSub != nil {
				panic(errSub)
			}
			procRsetSub := &ora.Rset{}
			_, errSub = stmtProcCallSub.Exe(procRsetSub)
			...
			//	recruitApplyMemberAnswerList := make([]models.RecruitApplyMemberAnswerList, 0)
			// recruitApplyMemberAnswer := models.RecruitApplyMemberAnswerList{}
			...
			if procRsetSub.IsOpen() {
				for procRsetSub.Next() {
					ansEntpMemNo = procRsetSub.Row[0].(string)
					ansRecrutSn = procRsetSub.Row[1].(string)
					ansQstSn = procRsetSub.Row[2].(string)
					ansVdTitle = procRsetSub.Row[3].(string)
					ansVdFilePath = procRsetSub.Row[4].(string)
					ansTotCnt = procRsetSub.Row[5].(int64)
					...
					imgServerFile := utils.CdnPathToImgSvrPath(ansVdFilePath, imgServer, rslRegDt)
					...
					recruitApplyMemberAnswerList = append(recruitApplyMemberAnswerList, models.RecruitApplyMemberAnswerList{
						...
						AnsTotCnt:           ansTotCnt,
						AnsVdFilePathImgSvr: imgServerFile,
					})

					// convDt, _ := time.Parse("20060102150405", rslRegDt)
					// dir, file := filepath.Split(ansVdFilePath)
					// imgServerFile := fmt.Sprintf("%v%vtemp_%v%v", imgServer, dir, convDt.Year(), file)

					fmt.Printf(fmt.Sprintf(" ===> PpMemNo:%v, VdPath: %v", rslPpMemNo, vdFullPtoPath))
					fmt.Printf(fmt.Sprintf(" ===> PpMemNo:%v, ImgServerVdPath: %v", rslPpMemNo, imgServerFile))
				}
				if errSub := procRset.Err(); errSub != nil {
					panic(errSub)
				}
			}
			// End : ZSP_QST_ANS_LIST_R

			fmt.Printf(fmt.Sprintf(" ===> EntpMemNo:%v, RecrutSn:%v, PpMemNo:%v, Name:%v, ApplyDt:%v, RegDt:%v, readEndDay:%v", rslEntpMemNo, rslRecrutSn, rslPpMemNo, rslNm, rslApplyDt, rslRegDt, readEndDay))
			...
			recruitStatList = append(recruitStatList, models.RecruitStatListAll{
				RslTotCnt:         rslTotCnt,
				RslEntpMemNo:      rslEntpMemNo,
				RslRecrutSn:       rslRecrutSn,
				RslFavrAplyPpYn:   rslFavrAplyPpYn,
		...
		rtnRecruitStatList = models.RtnRecruitStatListAll{
			Rtn:                    models.DefaultResult{RtnCd: 1, RtnMsg: "SUCCESS"},
