...
type MessageVerifyController struct {
	beego.Controller
}

func (c *MessageVerifyController) Post() {
	...
	fmt.Printf(fmt.Sprintf("CALL ZSP_MSG_VERIFY_R('%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pGbnCd, pVal, pEntpMemNo, pRecrutSn, pPpMemNo))
	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL ZSP_MSG_VERIFY_R('%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pGbnCd, pVal, pEntpMemNo, pRecrutSn, pPpMemNo),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
	)
