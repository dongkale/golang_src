...
type RecruitPostDetailPopupController struct {
	BaseController
}

func (c *RecruitPostDetailPopupController) Get() {
	...
	var (
		...
		jobfairTitle string // LDK 2020/08/26 : 추가
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			recruitModify = append(recruitModify, models.RecruitModify{
				...
				RecrutEdt:      recrutEdt, // LDK 2020/11/25: 즉시 마감 날짜 출력
			})
		}
	...
	*/
	...
	// LDK 2020/08/26 : 채용 정보 코드화, 추가 -->
	c.Data["JobfairTitle"] = jobfairTitle
	c.Data["CarrGbn"] = tables.MapCarrGbnCd[carrGbnCd]
	c.Data["EntpAddr"] = entpAddr
	c.Data["EmplTyp"] = tables.MapEmplTypCd[emplTypeCd]
	c.Data["LstEduGbn"] = tables.MapLstEduGbnCd[lstEduGbnCd]
	...
	c.TplName = "recruit/recruit_detail_popup.html"
}

func (c *RecruitPostDetailPopupController) Post() {
	// start : log
	log := logs.NewLogger()
	log.SetLogger(logs.AdapterConsole)
	// end : log
