...
type RecruitPostDetailExcelController struct {
	beego.Controller
}

func (c *RecruitPostDetailExcelController) Post() {
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			aleApplyDt = procRset.Row[0].(string)
			aleFavrAplyPpYn = procRset.Row[1].(string)
			aleNm = procRset.Row[2].(string)
			aleSex = procRset.Row[3].(string)
			aleBrthYmd = procRset.Row[4].(string)
			aleAge = procRset.Row[5].(string)
			aleEmail = procRset.Row[6].(string)
			aleMoNo = procRset.Row[7].(string)
			aleEvlPrgsStatNm = procRset.Row[8].(string)
			aleEvlStatDt = procRset.Row[9].(string)
			alePrgsStatCd = procRset.Row[10].(string)
			aleUpJobGrp = procRset.Row[11].(string)
			aleJobGrp = procRset.Row[12].(string)
			aleRecrutTitle = procRset.Row[13].(string)
			aleRecrutDy = procRset.Row[14].(string)
			aleLstEdu = procRset.Row[15].(string)
			aleCarrGbn = procRset.Row[16].(string)
			aleCarrDesc = procRset.Row[17].(string)
			aleFrgnLangAbltDesc = procRset.Row[18].(string)
			aleAtchDataPath = procRset.Row[19].(string)
			aleTechQlftDesc = procRset.Row[20].(string)
			aleAtchFilePathYn = procRset.Row[21].(string)
			aleVpYn = procRset.Row[22].(string)
			_ = procRset.Row[23].(string)
			scoreValue = procRset.Row[24].(int64)
			...
			cell.Value = aleVpYn
			cell = row.AddCell()
			cell.Value = fmt.Sprintf("%v", scoreValue)

			downloadPath = imgServer + "/excel/apply/" + pEntpMemNo.(string) + "/" + pRecrutSn + "/" + dateFmt + ".xlsx"
		...
		imgDir := uploadPath + "/excel/apply/" + pEntpMemNo.(string) + "/" + pRecrutSn + "/"
