...
type PwdFindCertController struct {
	BaseController
}

func (c *PwdFindCertController) Get() {
	...
	c.TplName = "common/pwd_find_cert.html"
}

func (c *PwdFindCertController) Post() {
	// start : log
	log := logs.NewLogger()
	log.SetLogger(logs.AdapterConsole)
	// end : log
	...
	fmt.Printf(fmt.Sprintf("CALL ZSP_FIND_PWD_STEP2_R('%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pMemId, pPpChrgNm, pEmail, pCertNo))
	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL ZSP_FIND_PWD_STEP2_R('%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pMemId, pPpChrgNm, pEmail, pCertNo),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
		ora.S,   /* MEM_ID */
