...
// LiveNvnCloseController ...
type LiveNvnCloseController struct {
	BaseController
}

// Post ...
func (c *LiveNvnCloseController) Post() {
	...
	if mem_no == nil {
		...
		c.Data["json"] = &models.LiveInvResult{}
		c.ServeJSON()
		return
	}
	...
	// Start : 종료 처리
	// 종료 처리 이므로 카운트 처리 않함 --> 0
	fmt.Printf(fmt.Sprintf("CALL ZMSP_LIVE_NVN_MSG_SEND_PROC('%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', %v, %v, :1)",
		pLang, pEntpMemNo, pRecrutSn, pApplyMemNoArr, pMsgGbnCd, pMsgCont, pLiveItvSdt, pArrPpChrgSn, pLiveSn, 0, 0))
	...
	fmt.Printf(fmt.Sprintf("CALL ZSP_LIVE_NVN_END_RSLT_R('%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pEntpMemNo, pRecrutSn, iPpMemNo, pLiveSn, pEntpGbn))
