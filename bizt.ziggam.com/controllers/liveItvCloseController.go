...
type LiveItvCloseController struct {
	BaseController
}

func (c *LiveItvCloseController) Post() {
	...
	// sessionLiveInv := c.GetSession("LiveInv")
	// if sessionLiveInv != nil {
	// 	c.SetSession("LiveInv", "")
	// 	c.SetSession("LiveInvTime", "")
	// }
	...
	// 종료 처리
	fmt.Printf(fmt.Sprintf("CALL MSP_LIVE_END_PROC('%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, "E", pEntpMemNo, pRecrutSn, pPpMemNo, pLiveSn))

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL MSP_LIVE_END_PROC('%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, "E", pEntpMemNo, pRecrutSn, pPpMemNo, pLiveSn),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
	)
	...
	//rtnLiveInvClose := models.DefaultResult{}
	...
	if rtnCd != 1 {
		logs.Error("CALL MSP_LIVE_END_PROC('%v', '%v', '%v', '%v', '%v', '%v', :1) ==> rtnCd != 1 : %v",
