...
type ApplicantDetailPopupP2Controller struct {
	BaseController
}

func (c *ApplicantDetailPopupP2Controller) Get() {
	...
	fmt.Printf(fmt.Sprintf("CALL ZSP_MEM_POP_INFO_R_V3('%v', '%v', '%v', '%v', :1)",
	...
	fmt.Printf(tables.MapLstEduGbnCd1[lstEduGbnCd1])
	fmt.Printf(tables.MapLstEduGbnCd2[lstEduGbnCd2])
	fmt.Printf(carrYear)
	fmt.Printf(tables.MapFrgnLangAbltCd1[frgnLangAbltCd1])
	fmt.Printf(tables.MapFrgnLangAbltCd2[frgnLangAbltCd2])
	...
	fmt.Printf(fmt.Sprintf("CALL ZSP_VP_LIST_R('%v', '%v', :1)", pLang, pPpMemNo))
	...
	c.TplName = "applicant/applicant_detail_popup_p2.html"
}
