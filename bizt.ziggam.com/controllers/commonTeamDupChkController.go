...
type CommonTeamDupChkController struct {
	beego.Controller
}

func (c *CommonTeamDupChkController) Post() {
	...
	fmt.Printf(fmt.Sprintf("CALL ZSP_TEAM_DUP_CHK_R( '%v', '%v', '%v', '%v', :1)",
		pLang, pGbnCD, pItemVal, pEntpMemNo))

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL ZSP_TEAM_DUP_CHK_R( '%v', '%v', '%v', '%v', :1)",
		pLang, pGbnCD, pItemVal, pEntpMemNo),
		ora.S, /* JOIN_YN */
	)
