...
type AdminEntpInfoTestController struct {
	BaseController
}

func (c *AdminEntpInfoTestController) Get() {
	...
	pSyyyy := c.GetString("s_yyyy")
	pSmm := c.GetString("s_mm")
	pSdd := c.GetString("s_dd")
	pEyyyy := c.GetString("e_yyyy")
	pEmm := c.GetString("e_mm")
	pEdd := c.GetString("e_dd")
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			memStatCd = procRset.Row[28].(string)
			useYn = procRset.Row[29].(string)
			bizRegFilePath = procRset.Row[30].(string)
			loginDt = procRset.Row[31].(string)
	...
	c.Data["pVdYn"] = pVdYn
	c.Data["pSyyyy"] = pSyyyy
	c.Data["pSmm"] = pSmm
	c.Data["pSdd"] = pSdd
	c.Data["pEyyyy"] = pEyyyy
	c.Data["pEmm"] = pEmm
	c.Data["pEdd"] = pEdd
	c.Data["pKeyword"] = pKeyword

	c.TplName = "admin/entp_info_test.html"
}
