...
type EntpInsertLoginController struct {
	beego.Controller
}

func (c *EntpInsertLoginController) Post() {
	...
	log.Debug("CALL SP_EMS_ENTP_REG_LOGIN_PROC( '%v', '%v', :1)",
		pLang, pMemId)

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_EMS_ENTP_REG_LOGIN_PROC( '%v', '%v', :1)",
		pLang, pMemId),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
		ora.S,   /* MEM_NO */
