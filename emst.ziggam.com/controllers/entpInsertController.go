...
type EntpInsertController struct {
	beego.Controller
}

func (c *EntpInsertController) Post() {
	...
	pEntpMemId := c.GetString("entp_mem_id")
	...
	*/
	...
	log.Debug("CALL SP_EMS_ENTP_REG_PROC('%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pEntpKoNm, pBizRegNo, pRepNm, pPpChrgNm, pPpChrgTelNo, pEntpMemId, sha_pwd, pEmail, pEmailRecvYn)
	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_EMS_ENTP_REG_PROC('%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pEntpKoNm, pBizRegNo, pRepNm, pPpChrgNm, pPpChrgTelNo, pEntpMemId, sha_pwd, pEmail, pEmailRecvYn),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
		ora.S,   /* CERT_KEY */
	...
	rtnEntpInsert := models.RtnEntpInsert{}
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			entpMemId = procRset.Row[3].(string)
		}
		...
		/*
			if rtnEntpInsert.RtnCd == 1 {
				m := gomail.NewMessage()
				m.SetHeader("From", "no-reply@ziggam.com")
				m.SetHeader("To", pEmail)
				m.SetHeader("Subject", "[직감:ZIGGAM] "+pEntpKoNm+"님의 기업회원 인증 메일입니다.")
				m.SetBody("text/html", `
				<html>
				<head>
					<meta charset="UTF-8">
					...
					<title>[직감]기업회원 인증(가입승인 대기)</title>
					<style>
				...
				<body>
					...
					<table width="80%" cellspacing="0" cellpadding="0" bgcolor="#ffffff" style="margin:auto;"  summary="이메일 인증을 위한 안내 메일입니다.">
						<caption style="display:none;">직감 가입승인 대기</caption>
						<thead>
							<tr>
							...
							<tr>
								<td>
									<div style="width:86%;margin:auto;padding:40px 15px 10px 15px;font-size:20px; text-align:center;">
										<strong><span style="color:#00CAED;">`+pEntpKoNm+`</span></strong>의 기업회원 가입을 위해<br>
										이메일 주소를 인증해 주세요.
									</div>
								</td>
							</tr>
						...
						<tbody>
							<tr>
								<td height="44" bgcolor="#00CAED" valign="middle" align="center" style="border-radius: 30px;">
									<a href="`+siteUrl+`/common/email/cert?mem_id=`+pEntpMemId+`&cert_key=`+rtnEntpInsert.EmailCertKey+`" style="color:#fff;font-size:15px;text-decoration: none;">인증하기</a>
								</td>
							</tr>
						</tbody>
					</table>
