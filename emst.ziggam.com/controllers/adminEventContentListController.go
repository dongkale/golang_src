...
type AdminEventContentListController struct {
	BaseController
}

func (c *AdminEventContentListController) Get() {
	...
	// Start : Event Content List
	log.Debug("CALL SP_EMS_EVENT_LIST_R('%v', %v, %v, :1)",
		pLang, pOffSet, pLimit)

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_EMS_EVENT_LIST_R('%v', %v, %v, :1)",
		pLang, pOffSet, pLimit),
		ora.I64, /* TOT_CNT */
		ora.I64, /* SN */
		ora.S,   /* GBN_NM */
	...
	eventContentList := make([]models.EventContentList, 0)
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			eventContentList = append(eventContentList, models.EventContentList{
				TotCnt: totCnt,
				Sn:     sn,
				GbnNm:  gbnNm,
				Title:  title,
	...
	// End : Event Content List

	c.Data["EventContentList"] = eventContentList
	c.Data["Pagination"] = pagination
	c.Data["PageNo"] = pageNo
	c.Data["MenuId"] = "04"
	c.Data["SubMenuId"] = "02"

	c.TplName = "admin/event_content_list.html"
}
