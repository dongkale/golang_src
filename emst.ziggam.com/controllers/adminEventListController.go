...
type AdminEventListController struct {
	BaseController
}

func (c *AdminEventListController) Get() {
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			vdCntString = procRset.Row[11].(string)
	...
	c.TplName = "admin/event_list.html"
}

func (c *AdminEventListController) Post() {
