...
func (c *FavorMemberUpdateController) Post() {
	...
	pFavrAplyPpYn := c.GetString("favr_aply_pp_yn")
	...
	log.Debug("CALL SP_EMS_FAVOR_APPLY_MEMBER_PROC('%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pEntpMemNo, pRecrutSn, pPpMemNo, pFavrAplyPpYn)
	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_EMS_FAVOR_APPLY_MEMBER_PROC('%v', '%v', '%v', '%v', '%v', :1)",
		pLang, pEntpMemNo, pRecrutSn, pPpMemNo, pFavrAplyPpYn),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
	)
