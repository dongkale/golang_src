...
type AdminNoticeListController struct {
	BaseController
}

func (c *AdminNoticeListController) Get() {
	...
	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_EMS_NOTICE_LIST_R('%v','%v', '%v', %v, %v, :1)",
		...
		ora.S,   /* EPS_YN */
		ora.S,   /* PUSH_SEND_DT */
		ora.I64, /* PUSH_SEND_CNT */
		ora.I64, /* PUSH_SEND_RES */
	)
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			pushDt = procRset.Row[9].(string)
			pushCnt = procRset.Row[10].(int64)
			pushResCnt = procRset.Row[11].(int64)
			pushResTotalCnt = procRset.Row[12].(int64)
			...
			noticeList = append(noticeList, models.NoticeList{
				...
				PushDt:          pushDt,
				PushCnt:         pushCnt,
				PushResCnt:      pushResCnt,
				PushResTotalCnt: pushResTotalCnt,
			})
		}
	...
	c.TplName = "admin/notice_list.html"
}
