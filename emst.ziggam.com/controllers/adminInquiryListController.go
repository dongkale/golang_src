...
type AdminInquiryListController struct {
	BaseController
}

func (c *AdminInquiryListController) Get() {
	...
	// Start : Inquiry List
	log.Debug("CALL SP_EMS_ADMIN_INQUIRY_LIST_R('%v', '%v', %v, %v, :1)",
		pLang, pEntpMemNo, pOffSet, pLimit)

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_EMS_ADMIN_INQUIRY_LIST_R('%v', '%v', %v, %v, :1)",
		pLang, pEntpMemNo, pOffSet, pLimit),
		ora.I64, /* TOT_CNT */
		ora.I64, /* BRD_NO */
		ora.S,   /* INQ_GBN_NM */
	...
	adminInquiryList := make([]models.AdminInquiryList, 0)
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			entpMemNo = procRset.Row[11].(string)
			entpKoNm = procRset.Row[12].(string)
			inqGbnCd = procRset.Row[13].(string)
			regDy = procRset.Row[14].(string)

			adminInquiryList = append(adminInquiryList, models.AdminInquiryList{
				TotCnt:    totCnt,
				BrdNo:     brdNo,
				InqGbnNm:  inqGbnNm,
				RegDt:     regDt,
	...
	c.Data["AdminInquiryList"] = adminInquiryList
	c.Data["Pagination"] = pagination
	c.Data["PageNo"] = pageNo
	c.Data["MenuId"] = "04"
	c.Data["SubMenuId"] = "03"

	c.TplName = "admin/inquiry_list.html"
}
