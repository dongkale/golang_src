...
func (c *UeditorController) ControllerUE() {
	...
	case "uploadimage", "uploadfile", "uploadvideo":
		...
		ueFolderNm := input.Get("ueFolderNm")       //생성폴더명
		ueSubFolderNm := input.Get("ueSubFolderNm") //서브생성폴더명

		nowDate := time.Now()
		dateFmt := fmt.Sprintf(nowDate.Format("20060102150405"))

		err = os.MkdirAll(uploadPath+"/attachment/"+ueFolderNm+"/"+ueSubFolderNm, 0777) //..代表本当前exe文件目录的上级，.表示当前目录，没有.表示盘的根目录
		if err != nil {
			logs.Error(err)
		}
		//保存上传的图片
		...
		path1 := uploadPath + "/attachment/" + ueFolderNm + "/" + ueSubFolderNm + "/" + dateFmt + "_" + h.Filename
		err = c.SaveToFile("upfile", path1) //.Join("attachment", attachment)) //存文件    WaterMark(path)    //给文件加水印
		if err != nil {
			logs.Error(err)
		}
		c.Data["json"] = map[string]interface{}{"state": "SUCCESS", "url": viewpath + "/attachment/" + ueFolderNm + "/" + ueSubFolderNm + "/" + dateFmt + "_" + h.Filename, "title": dateFmt + "_" + h.Filename, "original": dateFmt + "_" + h.Filename}
	...
	case "uploadscrawl":
		...
		//path1 := uploadPath + "/attachment/" + number + name + "\\"
		path1 := uploadPath + "/attachment/" + ueFolderNm + "/" + ueSubFolderNm + "/"
		//保存上传的图片
		...
		c.Data["json"] = map[string]interface{}{
			"state":    "SUCCESS",
			"url":      viewpath + "/attachment/" + ueFolderNm + "/" + ueSubFolderNm + "/" + newname + ".jpg",
			"title":    newname + ".jpg",
