...
type JobfairEntpRegisterController struct {
	beego.Controller
}

func (c *JobfairEntpRegisterController) Post() {
	...
	pJobfair := c.GetString("job_fair")      // 박람회 번호 배열 리스트
	...
	// Start : Jobfair Entp Register
	logs.Debug(fmt.Sprintf("CALL SP_JOBFAIR_ENTP_REG('%v', '%v', '%v', :1)",
		pLang, pEntpMemNo, pJobfair))

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL SP_JOBFAIR_ENTP_REG('%v', '%v', '%v', :1)",
		pLang, pEntpMemNo, pJobfair),
		ora.I64, /* RTN_CD */
		ora.S,   /* RTN_MSG */
	)
	...
	c.Data["json"] = &models.JobFairEntpRegister{RslData: defaultResult, RslJobFairList: jobFairInfoList}
	c.ServeJSON()
}
