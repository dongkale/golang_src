...
func (c *ApiGetBannerDetailController) Get() {
	...
	// Start : Banner Detail
	log.Debug("CALL MNG_ADMIN_BANNER_DTL_R('%v', '%v', :1)",
		pLang, pBnrSn)

	stmtProcCall, err := ses.Prep(fmt.Sprintf("CALL MNG_ADMIN_BANNER_DTL_R('%v', '%v', :1)",
		pLang, pBnrSn),
		ora.S,   /* BNR_SUB_CN_01 */
		ora.S,   /* BNR_SUB_CN_01 */
		ora.S,   /* PUBL_SDY */
	...
	if procRset.IsOpen() {
		for procRset.Next() {
			...
			bnrSubTitle = procRset.Row[1].(string)
			sdy = procRset.Row[2].(string)
			edy = procRset.Row[3].(string)
			...
			thumbPtoPath = procRset.Row[13].(string)
			bnrKndCd = procRset.Row[14].(string)
			listBnrSn = procRset.Row[15].(string)
			listTitle = procRset.Row[16].(string)
			listPhotoPath = procRset.Row[17].(string)
			listThumbPtotoPath = procRset.Row[18].(string)
			listLinkUrl = procRset.Row[19].(string)
